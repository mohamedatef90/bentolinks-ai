...
- **AI:** Pluggable enrichment provider (`_shared/llm.ts`, Vault `LLM_PROVIDER` = `gemini` | `claude` | `openai` | `nvidia`, default `gemini`; **production runs `nvidia`**): NVIDIA `z-ai/glm-5.2` (integrate.api.nvidia.com, OpenAI-compatible; model overridable via Vault `LLM_MODEL`; json_schema with json_object retry; 140s timeout — the shared endpoint queues 1.5-3min; **auto-falls back to Gemini on failure/timeout** so items never stall; every enrichment stamps `raw_metadata.enriched_by` (e.g. `nvidia:z-ai/glm-5.2` / `gemini:fallback`) for SQL-verifiable provenance), Gemini `gemini-2.5-flash`, Claude `claude-haiku-4-5` (tool-use JSON), or OpenAI `gpt-4o-mini` (json_schema) — each needs its key in Vault (`NVIDIA_API_KEY`/`GEMINI_API_KEY`/`ANTHROPIC_API_KEY`/`OPENAI_API_KEY`). Embeddings (`gemini-embedding-001`), TTS, and the YouTube/PDF/reel multimodal fallbacks stay Gemini-only regardless of provider. All server-side, keys read via `public.get_secret` RPC.
